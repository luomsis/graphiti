...
test.describe('Fix verification: Knowledge upload', () => {
  test('Knowledge page renders with existing documents', async ({ page }) => {
    const errors: string[] = [];
    collectErrors(page, errors);
  ...
  test('Uploaded document appears in the table after processing', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/knowledge-upload.png', fullPage: true });
  });
