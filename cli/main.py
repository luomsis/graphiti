...
def main():
    ...
    import_parser.add_argument(
        '--embedding-dimension', type=int, default=1536, help='Embedding vector dimension (must match DB schema)'
    )
    ...
    # Export also needs embedding-dimension for schema validation
...
async def handle_command(args):
    ...
    if args.command == 'export':
        ...
        driver = PostgresAgeDriver(
            dsn=args.dsn, schema=args.schema, embedding_dimension=args.embedding_dimension
        )
        try:
