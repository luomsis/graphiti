...
def remap_entity_edge(record: dict, uuid_map: dict, new_group_id: str) -> dict:
    ...
    result['episodes'] = [uuid_map.get(ep, ep) for ep in record.get('episodes', [])]
    return result
...
def remap_episodic_node(record: dict, uuid_map: dict, new_group_id: str) -> dict:
    ...
    entity_edges may reference edges that were pruned before export;
    such dangling references are kept as-is (uuid_map.get fallback).
    """
    result = _remap_uuid(record, uuid_map, new_group_id)
    result['entity_edges'] = [uuid_map.get(e, e) for e in record.get('entity_edges', [])]
    return result
