...
from cli.constants import ALL_TABLES, GENERATED_FIELDS
...
_GENERATED_FIELDS = GENERATED_FIELDS

# Table processing order respecting FK dependencies: nodes first, then edges.
_TABLE_ORDER = ALL_TABLES

# Batch size for INSERT operations to reduce Python/DB round-trips.
_BATCH_SIZE = 500
...
async def import_group(
    ...
    for table_name in _TABLE_ORDER:
        ...
        # Pre-process all records: remap, filter, jsonify
        remapped_records: list[dict] = []
        ...
        for record in records:
            ...
            remapped_records.append(filtered)

        # Execute INSERTs in batches
        assert sql is not None
        for i in range(0, len(remapped_records), _BATCH_SIZE):
            batch = remapped_records[i:i + _BATCH_SIZE]
            for record in batch:
                await driver.execute_query(sql, params=record)
